...
CREATE TABLE IF NOT EXISTS public.room_categories (
  ...
  CONSTRAINT room_categories_image_paths_max_4 CHECK (cardinality(image_paths) <= 4)
);
...
DROP POLICY IF EXISTS "Public can view site media storage" ON storage.objects;
CREATE POLICY "Public can view site media storage"
  ON storage.objects FOR SELECT
  TO public
  USING (bucket_id = 'site-media');

DROP POLICY IF EXISTS "Admins can upload site media storage" ON storage.objects;
...
DROP POLICY IF EXISTS "Admins can update site media storage" ON storage.objects;
...
DROP POLICY IF EXISTS "Admins can delete site media storage" ON storage.objects;
